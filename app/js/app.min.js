...
(function () {
  ...
  function service() {
    ...
    jobListings.forEach(function (position) {
...
(function () {
	...
	angular.module('modal', []).directive('modal', directive);
...
(function () {
  ...
  angular.module('products').service('productsService', service);
...
(function () {
  ...
  //Custom components
  'navigation', 'header', 'footer', 'homePage', 'contactUs', 'about', 'products', 'application', 'prodForm', 'employment', 'modal',
  //Services
...
(function () {
	...
	function controller($scope, $state, ScrollingService, NavigationService) {
		...
		$scope.$on('openModal', function (emitInfo, data) {
			var mi = $scope.modalInfo;
			mi[data.modal] = true;
		});
		...
		$scope.closeModal = function (event) {
			var elementClasses = ['prodception-close-icon', 'prodception-overlay'];
			var isTargetElement = elementClasses.some(function (className) {
				return event.target.className.includes(className);
			});
			if (isTargetElement) {
				var mi = $scope.$parent.modalInfo;
...
angular.module('templates').run(['$templateCache', function ($templateCache) {
  $templateCache.put('app/js/components/C:/Users/mrter/Desktop/current-projects/prod-app/app/js/components/about/about.html', '<div id="about"><h2>What is Prodception?</h2><hr><p>Prodception is an umbrella company which acts as a covering to multiple businesses we label as Products. The conception of these businesses can either begin within Prodception or from an indiviual wanting funding and support. The way this looks can vary depending on the Product or Service presented. Prodceptions main focus is giving Products an identity of their own. After a Product is established Prodception continues providing support in marketing and expansion. As an umbrella company we are fully committed to seeing our Products grow and provide service to the general public.</p><hr><div class="history"><div class="president"><img height="400" src="app/images/scott_terry.jpg"><!-- <i>Scott Terry, President and Ceo of Prodception</i>\t --></div><div><p>Prodception was started in 2017 by Scott Terry in Chicago, IL. The company was born out of the idea of having multiple businesses, but needing a central place to manage the businesses. There was a need to balance the investment of each business without letting another fall behind. From that Prodception was born taking on 3 initial businesses owned by Scott Terry. Apptex, Vlace, and LockSpace.</p></div></div></div>');
  $templateCache.put('app/js/components/C:/Users/mrter/Desktop/current-projects/prod-app/app/js/components/application/application.html', '<div id="application" class="container"><div class="app-content"><h2>Contract our infrastructure</h2><p>Get your business in order by having the correct infastructure</p><i ng-click="openModal()">Learn More</i><hr></div><prod-form form-type="\'application\'" content="application.formContent" is-application="true" path="\'/client/application\'"></prod-form></div>');
  $templateCache.put('app/js/components/C:/Users/mrter/Desktop/current-projects/prod-app/app/js/components/contact-us/contact.html', '<div id="contact"><prod-form form-type="\'contact-us\'" content="contact.formContent" is-contact-us="true" path="\'/contact/message\'"></prod-form></div>');
  $templateCache.put('app/js/components/C:/Users/mrter/Desktop/current-projects/prod-app/app/js/components/employment/employment.html', '<div id="employment"><h1>Work at Prodception</h1><p>Prodception is a revolutionary company. Part of what makes it that is our amazing workers. Come Join us in being a company that is changing the way business is managed.</p><div class="Job Listings"><h3>Open Positions</h3><hr><b ng-if="!employment.positions.length">There are currently no open positions</b><div ng-if="employment.positions.length"><div class="position" ng-repeat="position in employment.positions"><div class="header" ng-click="position.toggleContentVisibility()"><h3 ng-bind="position.position"></h3><i class="prodception-down-arrow" ng-class="{\'upward-arrow\': position.showContent}"></i></div><hr><div class="details" ng-class="{\'show-details\': position.showContent}"><p class="description" ng-bind="position.description"></p><ul class="requirements"><h4 class="heading">Requirements:</h4><li ng-repeat="requirement in position.requirements"><span>-</span><span>{{requirement}}</span></li></ul><button ng-bind="\'Apply Now\'" ng-click="employment.openModal()"></button><hr></div></div></div></div></div>');
  $templateCache.put('app/js/components/C:/Users/mrter/Desktop/current-projects/prod-app/app/js/components/footer/footer.html', '<fieldset class="footer"><legend><img height="60" src="app/images/plog.png"></legend><p>Copyright &copy; 2017 Prodception</p><ul><li ng-repeat="item in footer.routes" ng-click="footer.navigate(item.route)" ng-bind="item.format"></li></ul></fieldset>');
  $templateCache.put('app/js/components/C:/Users/mrter/Desktop/current-projects/prod-app/app/js/components/form/form.html', '<div id="form" class="container"><h2 ng-bind="form.content.heading"></h2><p ng-bind="form.content.subheading"></p><form name="mainForm"><div ng-form name="{{ form.formType }}" class="input-container"><label ng-repeat="field in form.formConfigurations" for="{{field.id}}" class="col col-sm-12 col-md-6 col-lg-6">{{ field.formattedName}} <input class="form-control" type="{{ field.type}}" name="{{field.id}}" ng-model="form.formData[field.id]" minlength="{{field.minlength}}" maxlength="f{{ield.maxlength}}" required="{{field.required}}"></label><p ng-if="form.isApplication">Tell us about your business needs</p><label class="col col-sm-12 col-md-12 col-lg-12">Message<textarea class="form-control" ng-model="form.formData.message" minlength="10" rows="6"></textarea></label></div><button ng-bind="\'Submit\'" ng-click="form.submit()"></button></form></div>');
  $templateCache.put('app/js/components/C:/Users/mrter/Desktop/current-projects/prod-app/app/js/components/header/header.html', '');
  $templateCache.put('app/js/components/C:/Users/mrter/Desktop/current-projects/prod-app/app/js/components/home_page/home_page.html', '<div class="header"><div class="background"></div><div class="white-background"></div><div class="text-container"><div class="wrapper"><h3>Prodception</h3><p>The Conception of Great Product</p></div></div></div><div class="companyExplanation"><h2>The Central Hub for Product Conception and Production</h2><hr><p>The list of businesses under the Prodception umbrella is growing and will continually expand. With a new way of approaching company management Prodception changes the way businesses are started and the approach to expansion. Our Products and Services will always provide quality, excellence and effiecency.</p></div><div class="info-banner"><h2>Our Products</h2></div><div class="products"><div ng-repeat="product in homepage.products"><div class="title"><h3 ng-bind="product.title"></h3><hr></div><a ui-sref="products"><img ng-src="{{product.imgsource}}"></a><hr><p class="summary" ng-bind="product.summary"></p></div></div><div class="reach-out"><h2>Contract our Infastructure</h2><hr><div class="main-content"><div><img height="80" src="app/images/002-people.svg"></div><div><p>When starting a business it can be a bit unclear exactly what you need, who you need, and when you need them. Much money and time is wasted intially by not having the correct infastructure. We work to bring your ideas to life by listening to your needs and providing the infastructure that suits those needs.</p></div></div><a ui-sref="apply">Apply Now</a></div><div class="questions"><h2>Anymore Questions? Reach out to us</h2><hr><p>If you have any questions about Prodception, the services we offer or how to see your business brought to life and listed feel free to <a ui-sref="contact-us">reach out to us today.</a></p><a ui-sref="contact-us"><img src="app/images/email.svg"></a></div>');
  $templateCache.put('app/js/components/C:/Users/mrter/Desktop/current-projects/prod-app/app/js/components/navigation/navigation.html', '<div id="navigation"><div class="navigation-bar"><p ng-click="navigation.navigate(\'home\')">Prodception</p><i class="icon-menu" ng-show="!navigation.menuSettings.isMenuOpen" ng-click="navigation.toggleMenu()"></i> <i class="icon-minus" ng-show="navigation.menuSettings.isMenuOpen" ng-click="navigation.toggleMenu()"></i></div><div class="navigation-menu animate" ng-if="navigation.menuSettings.isMenuOpen"><ul><li ng-repeat="item in navigation.routes" ng-click="navigation.navigate(item.route)" ng-bind="item.format"></li></ul></div></div>');
  $templateCache.put('app/js/components/C:/Users/mrter/Desktop/current-projects/prod-app/app/js/components/products/products.html', '<div id="products"><h2>Prodception Products</h2><hr><p>Here is the list of some of the companies under our umbrella.</p><div class="product-list"><div ng-repeat="product in products.products"><h3 ng-bind="product.title"></h3><div class="product-image"><img height="120" ng-src="{{product.imgsource}}"></div><div class="product-summary"><h4>What is it?</h4><p ng-bind="product.summary"></p></div><a href="#">Check it Out</a><hr></div></div></div>');
  $templateCache.put('app/js/components/C:/Users/mrter/Desktop/current-projects/prod-app/app/modal-templates/application-modal.html', '<div id="application-modal"><div class="prodception-overlay" ng-click="closeModal($event)"></div><div class="modal-content"><div class="modal-header"><i class="prodception-close-icon" ng-click="closeModal($event)"></i></div><h2>Contrat our Infastructure</h2><hr><img src="app/images/006-brainstorm.svg"><p>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry\'s standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum.</p><hr><h2>Select Your Services</h2><hr><img src="app/images/007-presentation.svg"><p>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry\'s standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum.</p><hr><h2>All Consultations are Free</h2><hr><img src="app/images/004-money.svg"><p>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry\'s standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum.</p></div></div>');
  $templateCache.put('app/js/components/C:/Users/mrter/Desktop/current-projects/prod-app/app/modal-templates/employment-modal.html', '<div id="employment-modal"><div class="prodception-overlay" ng-click="closeModal($event)"></div><div class="modal-content"><div class="RadioButtons">Choose your Position<label>Position 1 <input type="radio" name="radiobutton"></label></div><hr><input type="file" name="resume"> <sub>Please insert only PDF files</sub><div><h2>Anything else you want us to know?</h2><textarea rows="10"></textarea></div></div></div>');
}]);